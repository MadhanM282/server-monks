...
router.get("", async (req, res) => {
    try {
        ...
        const types = ["grouping", "dressing", "inspiration", "games", "study_rooms", "coding"];
        ...
        let clubs = await Club.find(match).populate({ path: "creator_id" }).populate({ path: "subcription_user_id" }).sort(sort).lean().exec();
