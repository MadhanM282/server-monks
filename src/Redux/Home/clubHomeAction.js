...
export const clubListData = (data, toast, navigate) => (dispatch) => {
    ...
    axios.post("https://server-monks-backend.herokuapp.com/clubs", data).then(({ data }) => {
...
export const getClubData = (page,sort,filter) => (dispatch) => {
    dispatch(clubLoding());
    axios.get(`https://server-monks-backend.herokuapp.com/clubs?count=${page}&sortBy=createdAt&OrderBy=${sort}&type=${filter}`).then(({ data }) => dispatch(getClubList(data)))
...
export const updateClubListData = (data, id, toast, navigate) => (dispatch) => {
    ...
    axios.patch(`https://server-monks-backend.herokuapp.com/clubs/${id}`, data).then(({ data }) => {
